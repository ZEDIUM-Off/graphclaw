...
- `goo.md` - canonical `Graph of Operations` semantics and reusable `GoO` composition
...
- if the task is about graph-theory grounding, `ProjectionRegistry`, `ContextFrame`, `GoT`, `GoO`, or mono-agent loop orchestration, start with `graph-governed-agentics.md`, then move to `projection-governance.md`, `context-frame.md`, `got.md`, `goo.md`, or `agent-loop.md`.
