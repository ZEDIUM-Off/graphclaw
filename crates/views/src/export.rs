...
use crate::types::{ResolvedView, ResolvedViewExport};
use serde_json::json;
...
pub fn export_llm_compact(
    ...
    let edges_text: Vec<String> = resolved
        ...
        .collect();
    let kind_str = resolved.view_kind.as_deref().unwrap_or("projection");
    let structured = json!({
...
/// Build a ResolvedViewExport for the given format.
...
mod tests {
    ...
    use crate::ResolvedView;
    use memgraph::{GraphEdge, GraphNode};
    use std::collections::HashMap;

    fn sample_resolved_view() -> ResolvedView {
        ResolvedView {
