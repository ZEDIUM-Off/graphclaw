//! ViewsService: in-memory template store + bounded view resolution.

use crate::error::ViewsError;
use crate::types::{BoundView, ResolvedView, Selectors, ViewKind, ViewOperation, ViewTemplate};
...
impl MaterializedSet {
    ...
    fn project(&mut self, mode: &str) -> Result<(), ViewsError> {
        match mode {
            "identity" => {}
            ...
            other => {
                return Err(ViewsError::InvalidOperation(format!(
...
pub struct ViewsService {
    templates: Arc<RwLock<HashMap<String, ViewTemplate>>>,
...
impl ViewsService {
    ...
    pub async fn create_template(&self, template: ViewTemplate) -> Result<(), ViewsError> {
    ...
    pub async fn update_template(&self, template: ViewTemplate) -> Result<(), ViewsError> {
        let mut templates = self.templates.write().await;
        if !templates.contains_key(&template.id) {
            return Err(ViewsError::TemplateNotFound(template.id));
        }
        templates.insert(template.id.clone(), template);
        Ok(())
    }
    ...
    pub async fn get_template(&self, id: &str) -> Result<Option<ViewTemplate>, ViewsError> {
        let templates = self.templates.read().await;
        Ok(templates.get(id).cloned())
    }

    /// List all template ids.
    pub async fn list_templates(&self) -> Result<Vec<String>, ViewsError> {
        let templates = self.templates.read().await;
    ...
    /// Bind a template (produce a BoundView).
    pub async fn bind(&self, bound: BoundView) -> Result<BoundView, ViewsError> {
        let templates = self.templates.read().await;
        if !templates.contains_key(&bound.template_id) {
            return Err(ViewsError::TemplateNotFound(bound.template_id));
        }
        Ok(bound)
    }

    /// Resolve a bound view on the graph.
    pub async fn resolve(&self, bound: BoundView) -> Result<ResolvedView, ViewsError> {
        let template = self
            .load_merged_template(&bound.template_id, &mut Vec::new())
            .await?;
        let outcome = self
            .resolve_template(template.clone(), &bound, &mut Vec::new())
            .await?;

        let view_kind = Some(view_kind_name(&template.kind).to_string());
    ...
    ) -> Result<ResolutionOutcome, ViewsError> {
        if stack.iter().any(|id| id == &template.id) {
            return Err(ViewsError::TemplateCycle(template.id));
        }
    ...
    async fn apply_operation(
        ...
        match operation {
            ViewOperation::Union { view_ids } => {
                for view_id in view_ids {
                    let template = self.load_merged_template(view_id, &mut Vec::new()).await?;
                    let child = self.resolve_template(template, bound, stack).await?;
                    outcome.set.union_with(&child.set);
                    outcome.trace.push(format!("union {view_id}"));
                    outcome.degradations.extend(
                        child
                            .degradations
                            .into_iter()
                            .map(|degradation| format!("child {view_id}: {degradation}")),
                    );
                }
            }
            ViewOperation::Intersection { view_ids } => {
                ...
                for view_id in view_ids {
                    ...
                    has_child = true;
                    outcome.trace.push(format!("intersection {view_id}"));
                    outcome.degradations.extend(
                        child
                            .degradations
                            .into_iter()
            ...
            ViewOperation::Difference { a, b } => {
                let template_a = self.load_merged_template(a, &mut Vec::new()).await?;
                let template_b = self.load_merged_template(b, &mut Vec::new()).await?;
                let resolved_a = self.resolve_template(template_a, bound, stack).await?;
                let resolved_b = self.resolve_template(template_b, bound, stack).await?;
            ...
            ViewOperation::Slice { limit, order } => {
    ...
    ) -> Result<MaterializedSet, ViewsError> {
    ...
    #[async_recursion]
    async fn load_merged_template(
        &self,
        template_id: &str,
        stack: &mut Vec<String>,
    ) -> Result<ViewTemplate, ViewsError> {
        if stack.iter().any(|id| id == template_id) {
            return Err(ViewsError::TemplateCycle(template_id.to_string()));
        }
        stack.push(template_id.to_string());

        let template = self
            .get_template(template_id)
            .await?
            .ok_or_else(|| ViewsError::TemplateNotFound(template_id.to_string()))?;

        let mut bases = Vec::new();
        for parent_id in &template.extends {
            bases.push(self.load_merged_template(parent_id, stack).await?);
        }

        stack.pop();
        Ok(merge_templates(&template, &bases))
    }
}

fn merge_templates(template: &ViewTemplate, bases: &[ViewTemplate]) -> ViewTemplate {
...
fn bound_node_ids(bound: &BoundView, selectors: &Selectors) -> Vec<i64> {
    let mut ids = bound
        .anchors
        .get("node_ids")
...
fn bound_label(bound: &BoundView, selectors: &Selectors) -> Option<String> {
    bound
        .anchors
        .get("label")
        .and_then(|value| value.as_str())
...
fn bound_props(
    bound: &BoundView,
    selectors: &Selectors,
) -> Result<HashMap<String, serde_json::Value>, ViewsError> {
    if let Some(props) = bound.anchors.get("props") {
        ...
        return Err(ViewsError::Serialization(
...
fn apply_filter_expression(set: &mut MaterializedSet, predicate: &str) -> Result<(), ViewsError> {
...
fn apply_node_predicate(set: &mut MaterializedSet, predicate: &str) -> Result<(), ViewsError> {
    ...
    Err(ViewsError::InvalidPredicate(predicate.to_string()))
}

fn apply_edge_predicate(set: &mut MaterializedSet, predicate: &str) -> Result<(), ViewsError> {
...
fn view_kind_name(kind: &ViewKind) -> &'static str {
    match kind {
        ViewKind::Semantic => "semantic",
        ViewKind::Boundary => "boundary",
        ViewKind::Projection => "projection",
    }
}
...
#[cfg(test)]
mod tests {
    use super::{GraphBackend, ViewsService};
    use crate::{BoundView, Selectors, ViewKind, ViewOperation, ViewTemplate};
    use async_trait::async_trait;
    ...
    fn service() -> ViewsService {
        ViewsService::with_backend(Arc::new(FakeBackend {
            nodes: vec![
    ...
    async fn resolve_supports_label_selectors_and_closed_slice() {
        let service = service();
        service
            .create_template(ViewTemplate {
                id: "business".to_string(),
                name: "Business".to_string(),
                kind: ViewKind::Semantic,
                ...
                filters: vec![],
                operations: vec![ViewOperation::Slice {
                    limit: 2,
        ...
        let resolved = service
            .resolve(BoundView {
                template_id: "business".to_string(),
                anchors: HashMap::new(),
    ...
    async fn resolve_supports_union_difference_and_expand() {
        ...
        service
            .create_template(ViewTemplate {
                ...
                filters: vec![],
                operations: vec![ViewOperation::Expand {
                    relation_type: None,
                    depth: 1,
                }],
                cost_limit: None,
        ...
        service
            .create_template(ViewTemplate {
                ...
                filters: vec![],
                operations: vec![ViewOperation::Union {
                    view_ids: vec!["core".to_string()],
                }],
                cost_limit: None,
            })
            .await
        ...
        service
            .create_template(ViewTemplate {
                ...
                filters: vec![],
                operations: vec![ViewOperation::Difference {
